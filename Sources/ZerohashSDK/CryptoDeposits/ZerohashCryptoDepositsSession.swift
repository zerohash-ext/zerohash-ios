...
@MainActor
public class ZerohashCryptoDepositsSession {
    ...
    private let callbacks: CryptoDepositsCallbacks
    ...
    /// The route embeds the crypto-deposits web component + iframe.
    private static let appIdentifier = "crypto-deposits"
    ...
    /// Presents the crypto-deposits UI from the specified view controller
        ...
        let webVC = IntegrationsWebViewController(
            ...
            callbacks: IntegrationsWebViewCallbacks(
                ...
                onCompleted: { data, jsonString in
                    callbacks.onCompleted?(CryptoDepositsEvent(from: data, jsonString: jsonString))
                },
                onFailed: { data, jsonString in
                    callbacks.onFailed?(CryptoDepositsEvent(from: data, jsonString: jsonString))
                },
                // A status rather than an outcome, so it gets its own event type
                // and never touches onCompleted. Same payload the Fund flow
                // receives — both come from the shared integrations hook.
                onDepositStatus: { data, jsonString in
                    callbacks.onDeposit?(IntegrationsDepositEvent(from: data, jsonString: jsonString))
                },
