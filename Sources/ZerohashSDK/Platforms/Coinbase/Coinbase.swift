...
public struct Coinbase: AuthFlow, DepositFlow, BalanceFlow, WithdrawFlow {
    ...
    /// Generated by the scraper-mobile-library submodule; `make sync` copies it in.
    /// Edit it there, not here.
    private static let domHelpersJS: String = {
        guard let url = resourceBundle.url(
                forResource: "shared-dom-helpers",
                withExtension: "js"
              ),
              let body = try? String(contentsOf: url, encoding: .utf8)
        else {
            preconditionFailure(
                "Shared shared-dom-helpers.js missing from SDK bundle. " +
                "Check Package.swift declares " +
